...
本文说明如何把 IDC-ONEMAN 部署为一个 Cloudflare Worker。前端由 Worker Assets 提供，API、D1、Queues、Cron、HashPay、CLICD、Resend 邮件任务和 WebVNC 都在同一 Worker 项目中运行；原 Python/FastAPI 版本保持不变。

> 一键部署不需要提供 Cloudflare API Token。部署页面会要求你登录 Cloudflare 和 GitHub，并由 Cloudflare 在你的账号中创建资源。只有使用本地 Wrangler 或自动化脚本部署时才需要 OAuth 登录或 API Token。
...
4. 填写下面五项部署变量。前三个安全值必须分别随机生成；Resend 发件地址必须属于已验证域名。
5. 确认构建命令和部署命令，开始部署。
6. 等待构建完成，打开页面给出的 `https://<worker>.<subdomain>.workers.dev` 地址。
7. 首次打开会显示“创建管理员”。输入管理员邮箱、密码和第 4 步填写的 `ADMIN_BOOTSTRAP_TOKEN`。
...
请使用密码管理器保存 `MASTER_KEY`。丢失该值后，D1 中已经加密的 CLICD Token、HashPay 私钥、卡密和实例凭据无法恢复。不要在 Git、Issue、聊天记录、截图或 Wrangler 配置中写入真实 Secret。

首次管理员创建完成后，即使 `ADMIN_BOOTSTRAP_TOKEN` 仍存在，Bootstrap 接口也会因为用户表非空而返回 `409 already_initialized`。如需进一步收紧权限，可在 Cloudflare Dashboard 的 Worker 设置中删除该 Secret；不要删除 `SECRET_KEY` 或 `MASTER_KEY`。
...
官方机制说明见 [Cloudflare Deploy to Cloudflare buttons](https://developers.cloudflare.com/workers/platform/deploy-buttons/)。Deploy Button 只支持 Workers，不支持 Pages；因此本版本使用单 Worker + Worker Assets，而不是旧版的 Worker API + Pages 双项目结构。
...
- 保存套餐时会即时查询 `/api/v1/images/enabled?type=lxc|kvm`。
- 套餐的节点、虚拟化类型和镜像必须完全匹配。
- 再次保存设置时 Token 留空会保留现值。
- WebVNC 需要 CLICD 提供公网 `wss://`，并支持 `binary` 与 `clicd-vnc-ticket.*` 子协议。
...
Cloudflare 版本把容器详情视为交付契约，而不是可选显示字段。Queue 至少需要取得以下数据后，订单才会从 `provisioning` 变为 `fulfilled`：

- CLICD 容器 ID、节点、远端名称、运行状态和到期时间。
- 套餐要求的 IPv4/NAT IPv4 与 IPv6。
- 有网络的套餐对应的 SSH 端口、用户名和初始密码。
- CLICD 子用户的管理用户名、初始密码、访问码和管理链接。

详情尚未齐全时，D1 保存已取得的数据与缺失项，Queue 使用同一容器 ID 重试，不会重复创建容器；客户界面显示“详情同步中”，也不会开放详情、重置密码或 VNC。`/api/instances/:id/access` 只有在完整性检查通过后才返回凭据。
...
Deploy Button 会收集以下两项；手动部署时在 Cloudflare Dashboard 的 Variables and Secrets 中新增：
...
1. 将 Gitee 上游 `oneman-idc/idc-oneman` 添加为 upstream，并审阅 Cloudflare 适配差异。
2. 合并或审阅上游提交，不要覆盖自己保存的业务代码。
3. 确认 `wrangler.jsonc` 中的 D1 和 Queue binding 名称没有意外变化。
4. 推送到连接 Cloudflare 的生产分支。
5. 在 Workers Builds 中确认 `npm run build`、D1 migration 和 `wrangler deploy` 全部成功。
6. 验证 `/healthz`、登录、订单、Queue backlog 和后台任务。

不要更换现有 `SECRET_KEY` 和 `MASTER_KEY`。正常代码更新不需要轮换这两个值；直接替换会使现有 Session 失效，并导致已加密业务数据无法解密。
...
一键按钮不需要 API Token。只有不使用 Deploy Button、直接从本机部署时，才需要执行：
...
- Deploy Button 使用 `main/cloudflare` 子目录。
- Worker、D1、Jobs Queue 和 Dead-letter Queue 位于同一 Cloudflare Account。
- D1 migration 全部 applied。
- `SECRET_KEY`、`MASTER_KEY`、`ADMIN_BOOTSTRAP_TOKEN` 是三个不同的强随机值；Resend Token 与已验证发件地址已填写。
- 首位管理员已通过初始化页面创建，普通注册随后可用。
- `/healthz`、`/api/config` 和前端静态资源从同一域名返回。
- CLICD LXC/KVM 镜像按节点正确过滤。
- 钱包与 HashPay 购买后，订单在容器详情完整前保持 `provisioning`；完成后容器 ID、名称、状态、到期时间、套餐要求的 IP、SSH 和管理凭据均非空。
- HashPay 回调地址为 `/api/payments/hashpay/callback`，重复回调不重复入账。
- Resend 测试邮件可投递，卡密正文只出现在邮件中，页面只显示脱敏摘要。
- 退款经过邮箱确认码和后台审核，容器销毁后再退回钱包。
- DEFAULT、NEW SKIN、GLASS UI 的中英文和明暗切换在桌面、移动端均无溢出。
- KVM VNC 在生产域名下完成真实连接测试。
- Workers Logs、D1 备份和 Dead-letter Queue 告警已启用。
...
6. Resend 控制台或收件箱：测试邮件已投递，仅显示 Message ID/主题/收件域名，隐藏地址本地部分和邮件正文敏感值。
7. 购买交付中：订单为 `provisioning`，实例卡显示“正在核对容器完整信息”，详情操作未开放。
8. 购买交付完成：订单为 `fulfilled`；容器 ID、远端名称、状态、到期时间、套餐要求的 IP、SSH 端口均可见。
9. 容器详情：SSH 与管理字段全部存在；截图中对用户名以外的凭据做遮挡。
10. CORS 与界面：同源请求成功、未授权 Origin 返回 `403 origin_not_allowed`；DEFAULT/NEW SKIN/GLASS UI 在桌面与移动端无横向溢出。

真实购买验收需要可创建测试容器的 CLICD 节点、HashPay 测试商户（或可控的真实小额支付）、已验证域名的 Resend API Token，以及可部署 Workers/D1/Queues 的 Cloudflare Account。建议全部使用专用测试资源，验收结束后轮换 Token 和测试凭据。
